      ...
      <div className="p-5 space-y-6">
        <div className="bg-primary p-6 rounded-[2rem] text-white space-y-4 relative overflow-hidden">
          ...
          <div className="flex justify-between items-start relative z-10">
            ...
            <span className={cn("px-3 py-1 backdrop-blur-md text-[10px] font-black rounded-full uppercase", event.status === 'Cancelled' ? "bg-red-500 text-white" : "bg-white/20 text-white")}>
              {event.status}
            </span>
          </div>
        ...
        <div className="space-y-3">
          ...
          <button onClick={handleShare} className="w-full py-4 rounded-2xl flex items-center justify-center gap-3 bg-[#00A859] text-white font-bold text-sm transition-transform active:scale-[0.98]">
           ...
           {event.status !== 'Cancelled' && (
             <button onClick={handleCancelClick} className="w-full py-4 bg-danger/10 text-danger rounded-2xl font-black text-sm uppercase tracking-widest mt-4 active:scale-95 transition-transform">
               Cancel Event
             </button>
           )}
         </div>
