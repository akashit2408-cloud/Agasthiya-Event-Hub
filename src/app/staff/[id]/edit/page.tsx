      ...
      try {
        ...
        if (data) {
          setFormData({
            name: data.name || "",
            phone: data.mobile || "",
            gpayNumber: data.gpay_number || "",
            role: data.role || "Helper",
            status: data.status || "Available",
            profileImage: data.avatar_seed || null
          });
          if (data.avatar_seed) {
            setImagePreview(data.avatar_seed);
          }
        }
      } catch (err) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <div className="bg-white p-5 flex items-center justify-between border-b border-gray-100 sticky top-0 z-10 shadow-sm">
        ...
        <h1 className="text-base font-extrabold text-gray-900">Edit Staff</h1>
